...
def list_bounding_box(persons : list):
    """ 
    function that fill out a list with the coordonates of the circle
    persons : list = list of persons in the simulation
    output:
        box : list
    ...
    box=[]
    for p in persons:
        box.append((p[0]-constants.PERSON_RADIUS, p[1]-constants.PERSON_RADIUS, p[0]+constants.PERSON_RADIUS, p[1]+constants.PERSON_RADIUS))
    return box


def circleCollision(c1, c2, coord : list):
    '''
    ...
    output :
        bool
    '''  
    if coord[c1][0]>coord[c2][2] or coord[c1][1]>coord[c2][3] or coord[c1][2]<coord[c2][0] or coord[c1][3]<coord[c2][1] :
        return False
...
def computeCollisions(persons, personNumber, coord : list):
    '''
    ...
    for j in range (len(persons)):
        ...
        if ( j != personNumber) and p[2] == constants.INFECTED:
            
            # Test collision between the persons p and q
            if circleCollision(personNumber, j, coord):
